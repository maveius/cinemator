...
from resources.lib.filmwebpy.filmweb.parser.ObjectParser import ObjectParser
from resources.lib.filmwebpy.filmweb.func import get_text_or_none, get_datetime_or_none
...
class MovieParser(ObjectParser):
    ...
    def _parse_basic(self):
        ...
        film_title = self.soup.find("div", {'class': "filmMainHeader"})
        try:
            title = film_title.find('a')

            dic['title'] = get_text_or_none(title)

            year = film_title.find('span', {'class': 'halfSize'})
            dic['year'] = get_text_or_none(year, 'int')

            s = film_title.find('h2')
            ...
            grabber = HTMLGrabber()
            poster_content = grabber.retrieve(self.obj.url+"/posters")
            poster_soup = BeautifulSoup(poster_content, "html.parser")
            poster = poster_soup.find('meta', {'property': 'og:image'})
            if poster:
                poster_img = poster.get('content')
