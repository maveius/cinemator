...
from resources.lib.filmwebpy.filmweb.Movie import Movie
from resources.lib.filmwebpy.filmweb.Person import Person
from resources.lib.filmwebpy.filmweb.parser.HTMLGrabber import HTMLGrabber
from resources.lib.filmwebpy.filmweb.vars import filmweb_search_blank
from resources.lib.filmwebpy.filmweb.vars import filmweb_person_search
from resources.lib.filmwebpy.filmweb.func import get_real_id
...
class FilmwebHTTP(object):
    ...
    def search_filtered_movie(self, title=None, page=1, genre_id=None, search_type=None):
        if search_type not in ['film', 'serial']:
            search_type = None

        return [Movie(objID=movieID, title=title, url=url, results_count=results_count) for movieID, title, url, results_count in
                self._search_filtered_movie(title, page, genre_id, search_type)]

    def _search_filtered_movie(self, title, page, genre_id, search_type):
        ...
        results_count = soup.find('div', {'id': 'resultsCount'})
        results_count = results_count.get_text().split('z')[1].strip()
        ...
        for hit in hits:
            ...
            yield movieID, title, url, results_count
