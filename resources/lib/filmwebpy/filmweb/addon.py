...
from resources.lib.filmwebpy.filmweb.vars import filmweb_root


class BaseObject(object):
    def __init__(self, objID, title=None, url=None, roleType=None, roleName=None, results_count=0):
        self.max_results = 10.0
        self.reset()
        self.set_id(objID)
        self.set_url(url)
        self.parser_obj = self.parser()
        self.set_title(title)
        self.pages = int(math.floor(float(results_count) / self.max_results))
